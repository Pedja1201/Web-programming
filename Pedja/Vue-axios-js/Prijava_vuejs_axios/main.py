...
def izmenaKorisnika(korisnik_oznaka):
    ...
    db = mysql.get_db()
    cursor = db.cursor()
    cursor.execute("UPDATE korisnik SET oznaka=%(oznaka)s, ime=%(ime)s, prezime=%(prezime)s, br_telefona=%(br_telefona)s WHERE oznaka=%(oznaka)s", korisnik)
    db.commit()
    cursor.execute("SELECT * FROM korisnik WHERE oznaka=%s", (korisnik_oznaka, ))
    korisnik = cursor.fetchone()
    return flask.jsonify(korisnik)
...
@app.route("/prijava")
def prjava():
    return app.send_static_file("prijava.html")
...
def izmenaPrijave(prijava_id):
    ...
    db = mysql.get_db()
    cursor = db.cursor()
    cursor.execute("UPDATE prijava SET korisnik_oznaka=%(korisnik_oznaka)s WHERE id=%(id)s", prijava)
    db.commit()
    cursor.execute("SELECT * FROM prijava WHERE id=%s", (prijava_id, ))
    prijava = cursor.fetchone()
    return flask.jsonify(prijava)
